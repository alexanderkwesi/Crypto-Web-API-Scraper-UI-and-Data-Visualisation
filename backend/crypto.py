...
def fetch_crypto_data(url):
    ...
    headers = {'Content-Type':'application/json', 'Accept': 'application/json', 'X-CMC_PRO_API_KEY': API_KEY}
    try:
...
@app.route('/api_runner', methods=['GET', 'POST'])
def api_runner():
    global df

    json_data = request.get_json()
    if not json_data or 'url' not in json_data:
        return "<div>Error: No URL provided</div>", 400

    url = json_data['url']
    if not is_valid_url(url):
        return "<div>Error: Invalid URL</div>", 400
    ...
    try:
        ...
        df = pd.json_normalize(result['data'])
        df['timestamp'] = pd.to_datetime('now').strftime('%Y-%m-%dT%H:%M:%SZ')
